      ...
      switch(page) {
          case 'main': return (
              <div className="animate-in fade-in duration-300">
                {(experimentalDesign || glassEffect || m3Design) ? (<div className="flex flex-col items-center pt-6 pb-8 px-6 bg-gradient-to-b from-primary/15 to-transparent"><div className="relative mb-6"><UserAvatarWithStatus user={currentUser as any} className={cn("w-28 h-28 text-4xl border-4 border-background shadow-2xl", m3Design ? "m3-bubble" : "rounded-full experimental-glow")} />{currentUser.activeGiftEmoji && <div className="absolute -bottom-1 -right-1 bg-background rounded-full w-10 h-10 flex items-center justify-center text-xl shadow-lg border-2 border-primary/20">{currentUser.activeGiftEmoji}</div>}</div><div className="text-center space-y-1.5"><h2 className="text-3xl font-bold font-headline flex items-center justify-center gap-2">{currentUser.name}{currentUser.isAdmin && <VerifiedBadge />}</h2><p className="text-muted-foreground font-medium">{currentUser.username}</p></div></div>) : (<div className="p-0 overflow-hidden"><UserProfileCard user={currentUser} onEditProfile={() => { onOpenChange(false); setTimeout(() => setShowEditProfile(true), 150); }} /></div>)}
                <div className={cn("border-t p-2 space-y-1", (experimentalDesign || glassEffect || m3Design) && "mt-2")}>
                  <SettingsItem icon={Paintbrush} label={t('appearance')} description={t('appearance_desc')} value={t(theme as any)} onClick={() => navigateTo('appearance')} showExpColors={experimentalDesign || glassEffect || m3Design} iconBg="bg-blue-500/15" iconColor="text-blue-500" glassEffect={glassEffect} />
                  <SettingsItem icon={MessageSquare} label={t('chat_settings')} description={t('chat_settings_desc')} onClick={() => navigateTo('chat')} showExpColors={experimentalDesign || glassEffect || m3Design} iconBg="bg-green-500/15" iconColor="text-green-500" glassEffect={glassEffect} />
                  <SettingsItem icon={ShieldCheck} label={t('privacy_security')} description={t('privacy_security_desc')} onClick={() => navigateTo('privacy')} showExpColors={experimentalDesign || glassEffect || m3Design} iconBg="bg-rose-500/15" iconColor="text-rose-500" glassEffect={glassEffect} />
                  <SettingsItem icon={HardDrive} label={t('data_storage')} description={t('data_storage_desc')} onClick={() => navigateTo('dataStorage')} showExpColors={experimentalDesign || glassEffect || m3Design} iconBg="bg-orange-500/15" iconColor="text-orange-500" glassEffect={glassEffect} />
                  <SettingsItem icon={Languages} label={t('language')} description={t('language_desc')} value={language.toUpperCase()} onClick={() => navigateTo('language')} showExpColors={experimentalDesign || glassEffect || m3Design} iconBg="bg-purple-500/15" iconColor="text-purple-500" glassEffect={glassEffect} />
                  <SettingsItem icon={InfGoldIcon} label="InfGold" description={t('infgold_desc')} onClick={() => navigateTo('infGold')} showExpColors={experimentalDesign || glassEffect || m3Design} iconBg="bg-amber-500/15" iconColor="text-amber-600" isGlow={experimentalDesign || glassEffect} glassEffect={glassEffect} />
                  <SettingsItem icon={User} label={t('profile')} description={t('edit_profile_desc')} onClick={() => navigateTo('account')} showExpColors={experimentalDesign || glassEffect || m3Design} iconBg="bg-teal-500/15" iconColor="text-teal-500" glassEffect={glassEffect} />
                  <SettingsItem icon={Star} label={t('whats_new')} description={t('whats_new_desc')} onClick={() => navigateTo('whatsNew')} showExpColors={experimentalDesign || glassEffect || m3Design} iconBg="bg-yellow-500/15" iconColor="text-yellow-600" glassEffect={glassEffect} />
                  <SettingsItem icon={HelpCircle} label={t('help')} description={t('faq_desc')} onClick={() => navigateTo('help')} showExpColors={experimentalDesign || glassEffect || m3Design} iconBg="bg-pink-500/15" iconColor="text-pink-500" glassEffect={glassEffect} />
                  <SettingsItem icon={RefreshCcw} label={t('check_updates')} description={t('check_updates_desc')} onClick={() => navigateTo('checkUpdates')} showExpColors={experimentalDesign || glassEffect || m3Design} iconBg="bg-indigo-500/15" iconColor="text-indigo-600" glassEffect={glassEffect} />
                  <SettingsItem icon={Info} label={t('version')} description={t('about_desc')} value={currentVersion} onClick={() => navigateTo('about')} showExpColors={experimentalDesign || glassEffect || m3Design} iconBg="bg-gray-500/15" iconColor="text-gray-500" glassEffect={glassEffect} />
                  {currentUser.isAdmin && (<div className='border-t mt-4 pt-2'><SettingsItem icon={Shield} label={t('admin_panel_title')} description={t('admin_panel_desc')} onClick={() => router.push('/admin')} showExpColors={experimentalDesign || glassEffect || m3Design} iconBg="bg-red-600/20" iconColor="text-red-600" isGlow={experimentalDesign || glassEffect} glassEffect={glassEffect} /></div>)}
                </div>
              </div>
          );
          case 'appearance': return (
            <div className='p-2 space-y-1 divide-y animate-in fade-in slide-in-from-right-4 duration-300'>
              ...
              <SettingsSwitchItem id="m3-design" label="Material 3 Design" checked={m3Design} onCheckedChange={toggleM3Design} description="Classic Google style layout." glassEffect={glassEffect} />
              {m3Design && isAndroid12Plus && (
                  <SettingsSwitchItem id="dynamic-palette" label="Включить динамическую палитру" checked={dynamicPalette} onCheckedChange={toggleDynamicPalette} description="Цвета системы (Material You)." glassEffect={glassEffect} />
              )}
                  ...
                  <div className="grid gap-3">
                      <div className={cn("flex items-center gap-4 p-5 border rounded-3xl shadow-sm", glassEffect ? "glass-panel" : "bg-card")}>
                          ...
                          <div className="flex-1">
                              <p className="font-black text-sm uppercase tracking-widest leading-none mb-1">Material 3</p>
                              <p className="text-[10px] text-muted-foreground font-medium leading-tight">Новый современный дизайн и динамические цвета.</p>
                          </div>
                      </div>
                      <div className={cn("flex items-center gap-4 p-5 border rounded-3xl shadow-sm", glassEffect ? "glass-panel" : "bg-card")}>
                      ...
                      <div className={cn("flex items-center gap-4 p-5 border rounded-3xl shadow-sm", glassEffect ? "glass-panel" : "bg-card")}>
                          <div className="w-12 h-12 rounded-2xl bg-green-500/10 flex items-center justify-center text-green-600 shrink-0"><Zap className="h-6 w-6" /></div>
                          <div className="flex-1">
                              <p className="font-black text-sm uppercase tracking-widest leading-none mb-1">Фоновое соединение</p>
                              <p className="text-[10px] text-muted-foreground font-medium leading-tight">Уведомления приходят быстрее при низком потреблении энергии.</p>
                          </div>
                      </div>
                  </div>
              </div>
          ...
          case 'about': return (
              <div className='p-12 flex flex-col items-center text-center gap-6 animate-in fade-in slide-in-from-right-4 duration-300'>
                <div className={cn("w-32 h-32 bg-primary flex items-center justify-center shadow-2xl shadow-primary/20", m3Design ? "m3-bubble" : "rounded-[2.5rem] experimental-glow")}><InfiniteLogo className='w-20 h-20 text-white' /></div>
