...
const SettingsItem = ({ icon: Icon, label, value, onClick, disabled = false, description, iconBg = "bg-primary/10", iconColor = "text-primary", showExpColors = false, isGlow = false, glassEffect = false }: { icon: React.ElementType, label: string, value?: string, onClick: () => void, disabled?: boolean, description?: string, iconBg?: string, iconColor?: string, showExpColors?: boolean, isGlow?: boolean, glassEffect?: boolean }) => (
    <button onClick={onClick} className={cn("flex items-center w-full p-4 text-left rounded-xl transition-all disabled:opacity-50 disabled:pointer-events-none group", glassEffect ? "glass-button mb-1 border-none shadow-none" : "hover:bg-muted")} disabled={disabled}>
        ...
        <div className="flex-1 flex flex-col justify-center min-w-0 mr-2"><span className="font-bold whitespace-normal leading-tight">{label}</span>{description && <span className="text-xs text-muted-foreground whitespace-normal leading-tight mt-0.5">{description}</span>}</div>
        <div className="flex items-center gap-2 text-muted-foreground shrink-0">{value && <span className='capitalize text-sm max-w-[80px] truncate font-bold'>{value}</span>}<ChevronRight className="h-5 w-5 shrink-0" /></div>
  </button>
);
...
export function ExperimentalSettingsDialog({ open, onOpenChange, currentUser }: { open: boolean, onOpenChange: (open: boolean) => void, currentUser: AuthenticatedUser }) {
  ...
  const transfersQuery = useMemo(() => { if (!db) return null; return query(collection(db, 'transfers'), where('senderId', '==', currentUser.uid), orderBy('timestamp', 'desc'), limit(50)); }, [db, currentUser.uid]); const { data: sentTransfers } = useCollection<Transfer>(transfersQuery);
  const receivedQuery = useMemo(() => { if (!db) return null; return query(collection(db, 'transfers'), where('receiverId', '==', currentUser.uid), orderBy('timestamp', 'desc'), limit(50)); }, [db, currentUser.uid]); const { data: receivedTransfers } = useCollection<Transfer>(receivedQuery);
  ...
  const isExperimentalDesignRestricted = useMemo(() => { if (typeof navigator !== 'undefined') { const ua = navigator.userAgent; const match = ua.match(/Android\s([0-9\.]+)/); const version = match ? parseFloat(match[1]) : null; return version !== null && version < 9; } return false; }, []);
  ...
  const handleBuyPrem = async (isYearly = false) => { if (!db || !currentUser.uid || isBuyingPrem) return; const cost = isYearly ? PREM_COST_YEARLY : PREM_COST; setIsBuyingPrem(true); try { await runTransaction(db, async (tx) => { const userRef = doc(db, 'users', currentUser.uid); const userSnap = await tx.get(userRef); if (!userSnap.exists()) throw new Error("User not found"); const data = userSnap.data(); const currentGold = data.infGoldBalance || 0; if (currentGold < cost) throw new Error(t('not_enough_gold')); tx.update(userRef, { infGoldBalance: increment(-cost), subscriptionTier: 'prem', subscriptionStartedAt: serverTimestamp(), showPremBadge: true }); }); toast({ title: t('dm_success'), description: "Infinite Prem activated!" }); handleBack(); } catch (e: any) { toast({ variant: 'destructive', title: 'Error', description: e.message }); } finally { setIsBuyingPrem(false); } };
  ...
  const handleLogout = async () => { if (auth && db && currentUser) { const userRef = doc(db, 'users', currentUser.uid); try { await updateDoc(userRef, { status: 'offline', lastSeen: serverTimestamp() }); } catch (error) { console.error(error); } auth.signOut(); } else if (auth) { auth.signOut(); } };
  const handleToggleLoginProtection = async (enabled: boolean) => { if (!db || !currentUser.uid) return; setIsUpdatingPrivacy(true); try { await updateDoc(doc(db, 'users', currentUser.uid), { loginProtectionEnabled: enabled }); toast({ title: t('dm_success') }); } finally { setIsUpdatingPrivacy(false); } };
  const handleDeleteAccount = async () => { if (!auth || !auth.currentUser || !db || !currentUser.uid) return; setIsDeletingAccount(true); try { const uid = currentUser.uid; const username = currentUser.username; sessionStorage.setItem('isDeletingAccount', 'true'); await runTransaction(db, async (transaction) => { transaction.update(doc(db, 'users', uid), { name: 'Deleted Account', username: `@deleted_${uid}`, avatar: '', status: 'offline', isDeleted: true, infGoldBalance: 0, subscriptionTier: 'none' }); if (username) { transaction.delete(doc(db, 'usernames', username)); } }); await auth.currentUser.delete(); toast({ title: t('delete_account_success') }); router.push('/goodbye'); } catch (e: any) { console.error(e); toast({ variant: 'destructive', title: 'Error', description: e.message || t('delete_account_error') }); sessionStorage.removeItem('isDeletingAccount'); } finally { setIsDeletingAccount(false); } };
  ...
  const botGuidePageContent = (
      <div className='p-6 space-y-8 animate-in fade-in slide-in-from-right-4 duration-300'>
          <div className='space-y-4'><h2 className='text-3xl font-black font-headline text-primary'>{t('bot_guide_title')}</h2><p className='text-muted-foreground text-sm leading-relaxed font-medium'>{t('bot_guide_intro')}</p></div>
          <div className='space-y-8'>
              <div className='space-y-3'><h3 className='font-black text-sm uppercase tracking-widest text-primary flex items-center gap-2'><Zap className='h-4 w-4 fill-primary/20' /> {t('bot_guide_events')}</h3><div className='grid gap-3'><div className='p-4 bg-muted/40 rounded-2xl border border-border/50'><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p className='text-xs leading-relaxed'>{processMarkdownChildren(children)}</p> }}>{t('bot_guide_event_start')}</ReactMarkdown></div><div className='p-4 bg-muted/40 rounded-2xl border border-border/50'><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p className='text-xs leading-relaxed'>{processMarkdownChildren(children)}</p> }}>{t('bot_guide_event_msg')}</ReactMarkdown></div></div></div>
              <div className='space-y-3'><h3 className='font-black text-sm uppercase tracking-widest text-blue-500 flex items-center gap-2'><MessageSquare className='h-4 w-4 fill-blue-500/20' /> {t('bot_guide_actions')}</h3><div className='grid gap-3'><div className='p-4 bg-muted/40 rounded-2xl border border-border/50'><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p className='text-xs leading-relaxed'>{processMarkdownChildren(children)}</p> }}>{t('bot_guide_action_send')}</ReactMarkdown></div><div className='p-4 bg-muted/40 rounded-2xl border border-border/50'><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p className='text-xs leading-relaxed'>{processMarkdownChildren(children)}</p> }}>{t('bot_guide_action_reply')}</ReactMarkdown></div><div className='p-4 bg-muted/40 rounded-2xl border border-border/50'><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p className='text-xs leading-relaxed'>{processMarkdownChildren(children)}</p> }}>{t('bot_guide_action_wait')}</ReactMarkdown></div></div></div>
              <div className='space-y-3'><h3 className='font-black text-sm uppercase tracking-widest text-purple-500 flex items-center gap-2'><Split className='h-4 w-4' /> {t('bot_guide_logic')}</h3><div className='p-4 bg-muted/40 rounded-2xl border border-border/50'><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p className='text-xs leading-relaxed'>{processMarkdownChildren(children)}</p> }}>{t('bot_guide_logic_if')}</ReactMarkdown><div className='mt-2 p-2 bg-black/5 rounded-lg font-mono text-[10px] opacity-70'>Example: {"{msg_text} == ping"} {'->'} Action: Send "pong"</div></div><div className='p-4 bg-muted/40 rounded-2xl border border-border/50'><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p className='text-xs leading-relaxed'>{processMarkdownChildren(children)}</p> }}>{t('bot_guide_logic_math')}</ReactMarkdown></div></div>
              <div className='space-y-3'><h3 className='font-black text-sm uppercase tracking-widest text-emerald-500 flex items-center gap-2'><ImageIcon className='h-4 w-4' /> {t('bot_guide_media')}</h3><div className='p-4 bg-muted/40 rounded-2xl border border-border/50'><p className='text-xs leading-relaxed'>{t('bot_guide_media_desc')}</p><div className='flex gap-2 mt-4'><div className='p-2 bg-muted rounded-lg'><ImageIcon className='h-3 w-3' /></div><div className='p-2 bg-muted rounded-lg'><Video className='h-3 w-3' /></div><div className='p-2 bg-muted rounded-lg'><Music className='h-3 w-3' /></div><div className='p-2 bg-muted rounded-lg'><FileText className='h-3 w-3' /></div></div></div></div>
              <div className='space-y-3'><h3 className='font-black text-sm uppercase tracking-widest text-rose-500 flex items-center gap-2'><Database className='h-4 w-4 fill-rose-500/20' /> {t('bot_guide_vars')}</h3><p className='text-[11px] font-bold text-muted-foreground/80 pl-1'>{t('bot_guide_var_intro')}</p><div className='space-y-1 text-xs font-mono bg-muted/60 p-4 rounded-2xl border border-rose-500/10'><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p>{processMarkdownChildren(children)}</p> }}>{t('bot_guide_var_user')}</ReactMarkdown><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p>{processMarkdownChildren(children)}</p> }}>{t('bot_guide_var_msg')}</ReactMarkdown><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p>{processMarkdownChildren(children)}</p> }}>{t('bot_guide_var_bot')}</ReactMarkdown><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p>{processMarkdownChildren(children)}</p> }}>{t('bot_guide_var_time')}</ReactMarkdown></div></div>
          </div>
      </div>
  );
  const mainPageContent = (
      <div className="animate-in fade-in duration-300">
        {(experimentalDesign || glassEffect) ? (<div className="flex flex-col items-center pt-6 pb-8 px-6 bg-gradient-to-b from-primary/15 to-transparent"><div className="relative mb-6"><UserAvatarWithStatus user={currentUser as any} className="w-28 h-28 text-4xl border-4 border-background shadow-2xl rounded-full experimental-glow" />{currentUser.activeGiftEmoji && <div className="absolute -bottom-1 -right-1 bg-background rounded-full w-10 h-10 flex items-center justify-center text-xl shadow-lg border-2 border-primary/20">{currentUser.activeGiftEmoji}</div>}</div><div className="text-center space-y-1.5"><h2 className="text-3xl font-bold font-headline flex items-center justify-center gap-2">{currentUser.name}{currentUser.isAdmin && <VerifiedBadge />}</h2><p className="text-muted-foreground font-medium">{currentUser.username}</p></div></div>) : (<div className="p-0 overflow-hidden"><UserProfileCard user={currentUser} onEditProfile={() => { onOpenChange(false); setTimeout(() => setShowEditProfile(true), 150); }} /></div>)}
  ...
  const handleSpin = async (): Promise<void> => {
    ...
    setTimeout(async () => { toast({ title: t('you_won'), description: `${winningPrize.value} InfGold!` }); setSpinning(false); try { await updateDoc(doc(db!, 'users', currentUser.uid), { infGoldBalance: increment(winningPrize.value), lastDailyBonusClaimed: serverTimestamp(), }); } catch (e) { console.error(e); } }, 5000);
  };
  const handleSetStoryExpiration = async (hours: number) => { if (!db) return; try { await updateDoc(doc(db, 'users', currentUser.uid), { storyExpirationDuration: hours }); toast({ title: t('dm_success') }); } catch (e) { console.error(e); } };
  const renderPage = () => {
      switch(page) {
          case 'main': return mainPageContent;
          case 'appearance': return (<div className='p-2 space-y-1 divide-y animate-in fade-in slide-in-from-right-4 duration-300'><SettingsSwitchItem id="dark-mode" label={t('dark_mode')} checked={isDarkMode} onCheckedChange={toggleTheme} description={t('light_mode')} glassEffect={glassEffect} /><SettingsItem icon={Paintbrush} label={t('color_theme')} value={t(theme as any)} onClick={() => navigateTo('theme')} description={t('color_theme')} glassEffect={glassEffect} /><SettingsSwitchItem id="sys-font" label={t('use_system_font_label')} checked={useSystemFont} onCheckedChange={toggleSystemFont} description={t('use_system_font_desc')} glassEffect={glassEffect} /><SettingsSwitchItem id="exp-design" label={t('experimental_design_label')} checked={experimentalDesign} onCheckedChange={toggleExperimentalDesign} description={isExperimentalDesignRestricted ? t('android_9_plus_only') : t('experimental_design_desc')} disabled={isExperimentalDesignRestricted} glassEffect={glassEffect} /><SettingsSwitchItem id="glass" label={t('glass_effect_label')} checked={glassEffect} onCheckedChange={toggleGlassEffect} description={t('glass_effect_desc')} glassEffect={glassEffect} /><SettingsSwitchItem id="show-feed" label={t('show_feed_label')} checked={showFeed} onCheckedChange={toggleShowFeed} description={t('show_feed_desc')} glassEffect={glassEffect} /></div>);
          ...
          case 'privacy': return (<div className='p-2 space-y-1 divide-y animate-in fade-in slide-in-from-right-4 duration-300'><SettingsSwitchItem id="login-prot" label={t('login_protection_label')} checked={!!currentUser.loginProtectionEnabled} onCheckedChange={handleToggleLoginProtection} description={t('login_protection_desc')} glassEffect={glassEffect} />{currentUser.loginProtectionEnabled && (<SettingsItem icon={Lock} label={t('cloud_password_label')} onClick={() => {}} description={t('cloud_password_desc')} glassEffect={glassEffect} />)}<div className={cn("p-4 space-y-3 rounded-xl", glassEffect && "glass-panel border-none")}><Label className="text-xs font-black uppercase tracking-widest text-muted-foreground ml-1">{t('story_expiration_label')}</Label><Select value={(currentUser.storyExpirationDuration ?? 24).toString()} onValueChange={(v) => handleSetStoryExpiration(parseInt(v))}><SelectTrigger className={cn("h-12 rounded-xl border-none font-bold", glassEffect ? "glass-input" : "bg-muted/50")}><SelectValue /></SelectTrigger><SelectContent className="rounded-xl"><SelectItem value="12">{t('story_expiration_12 hours')}</SelectItem><SelectItem value="24">{t('story_expiration_24 hours')}</SelectItem><SelectItem value="48">{t('story_expiration_48 hours')}</SelectItem><SelectItem value="72">{t('story_expiration_72 hours')}</SelectItem><SelectItem value="0">{t('story_expiration_never')}</SelectItem></Select></Select><p className="text-[10px] text-muted-foreground ml-1 leading-tight">{t('story_expiration_desc')}</p></div></div>);
          ...
          case 'help': return (<Accordion type="single" collapsible className="w-full animate-in fade-in slide-in-from-right-4 duration-300">{faqs.map((f, i) => (<AccordionItem value={`f-${i}`} key={i} className="px-4"><AccordionTrigger className="text-left font-bold">{f.question}</AccordionTrigger><AccordionContent><div className="prose prose-sm dark:prose-invert max-w-none">{f.answer.includes('[BOT_GUIDE_BUTTON]') ? (<> <ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p>{processMarkdownChildren(children)}</p> }}>{f.answer.replace('[BOT_GUIDE_BUTTON]', '')}</ReactMarkdown><Button variant="outline" className={cn("w-full rounded-xl h-12 mt-4 font-bold border-primary/20 hover:bg-primary/5", glassEffect && "glass-button border-none")} onClick={() => navigateTo('botGuide')}><Info className='mr-2 h-4 w-4 text-primary' /> {t('open_full_guide')}</Button></>) : (<ReactMarkdown remarkPlugins={[remarkGfm]} components={{ p: ({children}) => <p>{processMarkdownChildren(children)}</p> }}>{f.answer}</ReactMarkdown>)}</div></AccordionContent></AccordionItem>))}</Accordion>);
