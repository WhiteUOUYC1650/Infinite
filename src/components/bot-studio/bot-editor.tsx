...
export function BotEditor({ bot, onBack }: { bot: CustomBot, onBack: () => void }) {
  ...
  const onAddFromFab = (type: BotBlockType) => {
      ...
      const isUI = type.startsWith('ui_');
                    ...
                    {miniApps.map(app => (
                        <div key={app.id} className="p-6 bg-card border rounded-3xl shadow-sm space-y-4 relative group animate-in zoom-in duration-300" onClick={() => setEditingAppId(app.id)}>
                            <Button 
                                variant="ghost" 
                                size="icon" 
                                onClick={(e) => { e.stopPropagation(); removeMiniApp(app.id); }} 
                            ...
                            <div className="flex items-center gap-4 mb-2">
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    <Input 
                                        value={app.name} 
                                        onChange={e => { e.stopPropagation(); updateMiniApp(app.id, 'name', e.target.value); }} 
                ...
                <div className="max-w-2xl mx-auto space-y-8">
                    <div className="flex items-center justify-between border-b pb-4">
                        <div className="flex items-center gap-3">
                            <Button variant="ghost" size="icon" onClick={() => setEditingAppId(null)} className="rounded-full">
                                <ArrowLeft className="h-5 w-5" />
                            </Button>
                            <div>
                                <h2 className="text-xl font-black font-headline text-primary">{currentEditingApp.name}</h2>
                                <p className="text-[10px] font-bold uppercase text-muted-foreground">UI Builder</p>
                            </div>
                        </div>
                        <Button variant="outline" size="sm" onClick={() => setBlockSelectorOpen(true)} className="rounded-xl font-bold">
                            <Plus className="h-4 w-4 mr-2" /> Add UI Component
                        </Button>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {(!currentEditingApp.blocks || currentEditingApp.blocks.length === 0) && (
                            <div className="text-center py-20 bg-muted/20 rounded-3xl border-2 border-dashed opacity-40">
                                <LayoutGrid className="h-16 w-16 mx-auto mb-4" />
                                <p className="font-bold uppercase tracking-widest text-xs">No UI blocks added.</p>
                            </div>
                        )}
                    </div>
                </div>
          ...
          <DialogContent className="max-w-[95vw] sm:max-w-md rounded-3xl p-0 overflow-hidden flex flex-col h-[70vh]">
              ...
              <Tabs defaultValue={editingAppId ? "ui" : "events"} className="flex-1 flex flex-col overflow-hidden">
                  <TabsList className={cn("mx-6 mt-4 grid bg-muted/50 p-1 rounded-xl", editingAppId ? "grid-cols-1" : "grid-cols-4")}>
                      ...
                      {editingAppId && (
                          <TabsTrigger value="ui" className="rounded-lg text-[10px] uppercase font-bold px-1">Интерфейс</TabsTrigger>
                      )}
                  </TabsList>
            ...
            case 'ui_button':
                return (
                    <div className="space-y-2 mt-1 w-full">
                        <Textarea 
                            placeholder={block.type === 'ui_button' ? "Текст кнопки..." : "Введите сообщение..."} 
                            value={block.params?.text || ''} 
                            onChange={e => onUpdate(sIdx, bIdx, 'text', e.target.value)} 
                            className="min-h-[60px] bg-black/10 border-none text-white placeholder:text-white/40 font-bold text-xs w-full whitespace-pre-wrap resize-none overflow-hidden" 
                            rows={2} 
                ...
                {!isTrigger && (
                    <div className="flex items-center gap-1 shrink-0">
                        <Button 
                            ...
                            size="icon" 
                            disabled={isFirst} 
                            onClick={() => onMove(sIdx, bIdx, 'up', isMiniApp)}
                        ...
                        <Button 
                            ...
                            size="icon" 
                            disabled={isLast} 
                            onClick={() => onMove(sIdx, bIdx, 'down', isMiniApp)}
                            className="h-7 w-7 rounded-lg bg-black/10 hover:bg-black/20 text-white disabled:opacity-20"
                        >
                        ...
                        <Button 
                            variant="ghost" 
                            size="icon" 
                            onClick={() => onDelete(sIdx, bIdx, isMiniApp)}
