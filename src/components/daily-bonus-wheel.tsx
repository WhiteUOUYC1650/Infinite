...
export const PRIZE_CONFIG = [
  { value: 50, weight: 1.5, color: '#4285F4' },
    ...
    <div className="flex flex-col items-center gap-8 p-4">
      <div className="relative w-64 h-64 md:w-80 md:h-80">
        ...
        <div
          ...
          style={{ transform: `rotate(${rotation}deg)`, background: `conic-gradient(${conicGradient})`, transitionDuration: '5000ms' }}
        >
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-background border-2 border-muted-foreground z-20" />
            {PRIZES_WITH_ANGLES.map((prize, i) => {
                const midAngle = prize.startAngle + prize.angle / 2;
                return (
                    <div key={i} className="absolute top-0 left-0 w-full h-full flex justify-center items-start text-white font-bold text-lg" style={{ transform: `rotate(${midAngle}deg)` }}>
                        <span className="flex items-center gap-1 pt-4" style={{ transform: `rotate(${-midAngle}deg)`, textShadow: '0 1px 1px #000, 0 1px 5px #000' }}>
      ...
      {!isSpinning && canSpin && (<Button onClick={() => onSpin()} size="lg" className='w-48'>{t('spin_the_wheel')}</Button>)}
      {!isSpinning && !canSpin && (<Button disabled size="lg" className='w-48'>{t('come_back_tomorrow')}</Button>)}
